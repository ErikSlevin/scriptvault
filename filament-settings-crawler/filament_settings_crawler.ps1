...
$outputFilePath = "C:\Users\erikw\Documents\GitHub\3d-printing\filament\readme.md"
...
foreach($key in $propertyValues.Keys) {
    $translation = SucheUndSpeichereZeilennummern -Suchwort $key
...
foreach ($file in $files) {
    ...
    foreach ($line in $content) {
        ...
        if ($writing) {
            ...
            Add-Content -Path $outputFilePath -Value $line.Trim()
        }
    }
